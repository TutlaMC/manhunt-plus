...
public class CommandLootPoolCancel extends TutlaCommand  {
    private static final String usage = "/lootpool cancel";
    public CommandLootPoolCancel(){
        super("cancel", usage, "Cancel lootpool creation", CommandSection.LOOTPOOL,  null);
    }

    @Override
    public boolean run(CommandContext ctx){
        if(LootPoolCommandUtil.existsUninitialised(ctx)) return true;
        LootPoolManager.cancelUninitalisedLootPool();
        ctx.player.sendMessage(TextUtil.parse("<green>Cancelled lootpool creation!"));
        return false;
    }
}
